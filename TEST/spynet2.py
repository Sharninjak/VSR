...
def flow_warp(img, flow, padding_mode='zeros'):
    ...
    Args:
        img (Tensor): Input image or feature map with shape (n, c, h, w).
        flow (Tensor): Optical flow with shape (n, h, w, 2).
        padding_mode (str): Padding mode for outside grid values. Default: 'zeros'.
    
    Returns:
        Tensor: Warped image or feature map with the same shape as the input.
    """
    n, c, h, w = img.size()
    # Create mesh grid
    grid_y, grid_x = torch.meshgrid(torch.arange(0, h, device=img.device), torch.arange(0, w, device=img.device))
    grid = torch.stack((grid_x, grid_y), 2).float()  # Shape: (h, w, 2)
    grid = grid.unsqueeze(0).repeat(n, 1, 1, 1)  # Shape: (n, h, w, 2)

    # Add flow to the grid
    grid = grid + flow.permute(0, 2, 3, 1)  # Shape: (n, h, w, 2)
